<script lang="ts">
	...
	let keywords = $derived(keywordCtx.filter((kw) => batchData.keywordIds.includes(kw.keywordId)));
	...
	$effect(() => {
		...
		let commonKeywords = new Set<number>(keywordCtx.map((kw) => kw.keywordId));
		...
		<div class="fixed inset-x-0 bottom-0 bg-white text-black">
			...
			<div class="grid grid-cols-5 px-2 py-4 lg:grid-cols-10 lg:gap-4 lg:px-4 lg:py-2">
				...
				<SearchSelect
					...
					dataFilter={(tz) =>
						tz.zone.toLowerCase().includes(batchData.timezone?.toLowerCase() ?? '')}
					autoSelect={true}
				/>
				...
				<SearchSelect
					...
					onSelect={() => {
						...
						if (keywordData) {
							...
							deletedKeywordIds = deletedKeywordIds.filter((id) => id !== keywordData.keywordId);

							// Add the selected keyword's ID to the list if it doesn't exist yet
							if (!batchData.keywordIds.includes(keywordData.keywordId))
								batchData.keywordIds.push(keywordData.keywordId);
					...
					{#each keywords as keyword (keyword.keywordId)}
						<span class="bg-black py-1 pl-3 text-sm text-white">
							{keyword.keyword} |<button
								...
								onclick={() => {
									batchData.keywordIds = batchData.keywordIds.filter(
										(id) => id !== keyword.keywordId
									);
									deletedKeywordIds.push(keyword.keywordId);
								}}>&nbsp;X</button
							>
						</span>
					{/each}
